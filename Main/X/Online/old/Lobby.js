...
async function fetchLobbyMembers(lobbyId) {
    const response = await fetch(`https://powarznastrona.pythonanywhere.com/get_lobby_members?lobby_id=${lobbyId}`);
    if (!response.ok) throw new Error('Failed to fetch lobby members');
    return response.json();
}

async function fetchGameStarted(lobbyId) {
    const response = await fetch(`https://powarznastrona.pythonanywhere.com/is_game_started?lobby_id=${lobbyId}`);
...
function updateLobby(members, gameStarted) {
    if (gameStarted.game_started == true) {
        window.location.href = `https://powarznastrona.pythonanywhere.com/game?lobby_code=${lobbyCode}&member_name=${member_name}`
    }
